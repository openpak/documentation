...
Openpak has grown massively over the past few years, making it easier than ever to get a ton of apps all from one place—but with that growth comes new problems. We're here with a solution to ensure developers' apps, Openpak, and the entire Linux desktop ecosystem is presented in what we believe is the best light possible.

<!-- truncate -->

Today, Openpak has over 2,400 apps, including many of the big names you would expect. In the early days our goal was mostly to make it easy to get the latest version of popular apps via Flatpak, and we can safely say this has been achieved: not only can you get pretty much any app on Openpak, they are often directly maintained by the developer—and it's the primary channel many new app developers choose for shipping their apps.

![App cards with high quality metadata on the homepage](good-metadata-apps.png)

## More Apps, More Problems

However, having lots of apps comes with some new (in this case nice to have) problems. While there are a lot of beautiful modern apps, there are also a lot of apps with low quality metadata, ugly icons, or out of date screenshots. This means that looking at the new and updated lists on the Openpak home page you get the impression that most of the apps on Openpak are low quality.
...
So how can we make Openpak a place for people to discover awesome apps? We've thought about this quite a bit, and landed on a two-pronged approach:
...
We're also introducing a new review system for app metadata, where editors rate which of the criteria a specific app meets. As an app developer you can see the state of your metadata on the app page on Openpak.
...
If editors flagged issues with your app you can address them in the next release. Once the new release is on Openpak you can request a re-review. The status of your app will be updated accordingly.
...
This does not affect the app review process for apps being published on Openpak. We will continue to accept apps that don't fully meet the new quality guidelines. However, we do want to highlight apps that meet the guidelines, and try to show them in more prominent places on the website.
...
![Openpak home page mockup with app banners including screenshots](homepage-banners.png)
...
At this stage our focus is on the Openpak website, but of course eventually we want to make this data available to native app store clients as well.

This effort will also help to improve the Openpak-wide search and the discoverability and attractiveness of apps on Openpak from the rest of the internet.

![Mockup of GNOME Software with banners including screenshots](banner-native.png)

## What's Next?

As of today, if you have an app on Openpak you can log in on the website, and see the app metadata ratings for your apps. Please have a look at your apps' ratings and the new documentation and make sure your app's metadata is in shape when we start rolling out banners over the coming weeks :)

If you run into issues or have questions please contact us in [#openpak on Matrix](https://matrix.to/#/#openpak:matrix.org).
