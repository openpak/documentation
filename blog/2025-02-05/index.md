...
As the premier source of apps for Linux, Openpak serves a wide range of people across a huge variety of hardware: from ultra powerful developer workstations to thin and light tablets; from handheld gaming consoles to a growing number of mobile phones. Generally any app on Openpak will work on a desktop or laptop with a large display, keyboard, and mouse or trackpad. However, devices with only touch input and smaller screen sizes have more constraints.

## Revealing the App Ecosystem

Using [existing data][documentation] and open standards, we’re now highlighting apps on Openpak that report as being designed to work on these mobile form factors. This new [On the Go collection][collection] uses existing device support data submitted by app developers in their MetaInfo, the same spec that is used to build those app’s listings for Openpak and other app store clients. The collection is featured on the Openpak.org home page for all devices.
...
[collection]: https://openpak.org/apps/collection/mobile/
[documentation]: https://docs.openpak.org/docs/for-app-authors/metainfo-guidelines/#device-support
