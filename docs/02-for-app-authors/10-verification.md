...
Verification is the process by which Openpak and developers confirm that
...
## I'm publishing an app on Openpak. How do I get it verified?
...
To verify an app is associated with a specific domain, Openpak uses the
...
`https://example.com/.well-known/org.openpak.VerifiedApps.txt`.
...
authenticating with Openpak using an authorized account. The following
...
Apps already on Openpak with a `com.github.` or `com.gitlab.` app ID
...
The first step is to gain ownership over your app; see [Someone else has put my app on Openpak—what do I do?](/docs/for-app-authors/submission#someone-else-has-put-my-app-on-openpakwhat-do-i-do)
